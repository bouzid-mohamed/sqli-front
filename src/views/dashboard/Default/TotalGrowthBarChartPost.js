...
const TotalGrowthBarChart = (props) => {
    ...
    useEffect(() => {
        setLoad(true)
        setSerie([])
        CommandeServices.getPostStatics().then((res) => {
            ...
            res.data.filter((n) => {
                ...
                if (parseInt(n.year) === parseInt(year)) {
                    if (n.status === 'affectationPoste') {
                        ser3[parseInt(n.month) - 1] = n.nbrCmd
                    } else if (n.status === 'confirmationPoste') {
                        ser4[parseInt(n.month) - 1] = n.nbrCmd
                    } else if (n.status === 'finie') {
            ...
            setAnnee(tabAnnee)
            props.setGainTot(gt, enCours, totalRetour, nbrRetour, TotalLivreur, nbrLivreur, nbrGain, nbrEnCour, ChartDataMonth);
